...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, config = None) -> dict[str, Any]:
    """
    ...
    if "actions" in batch.batch:
        ...
        scale_mask = batch.batch.get("scale_mask", None)
        max_scale = config.max_scale
        min_scale = config.min_scale

        if config.use_discrete_action:
            step = 0.25
            steps = int((max_scale - min_scale) / step) + 1
            scale_bins = torch.linspace(min_scale, max_scale, steps, device=actions.device)
            scales = scale_bins[actions.long()]
        else:
            if not actions.is_floating_point():
                actions = actions.float()
            scales = min_scale + actions * (max_scale - min_scale)

        valid_samples_mask = None
        if scale_mask is not None:
            mask_f = scale_mask.float()
            sample_sums = (scales * mask_f).sum(dim=-1)
            sample_counts = mask_f.sum(dim=-1)
            valid_samples_mask = sample_counts > 0
        else:
            sample_sums = scales.sum(dim=-1)
            sample_counts = torch.full_like(sample_sums, scales.shape[-1])
            valid_samples_mask = torch.ones_like(sample_sums, dtype=torch.bool)
            
        if valid_samples_mask is not None and valid_samples_mask.any():
            sample_means = sample_sums[valid_samples_mask] / sample_counts[valid_samples_mask]

            metrics["critic/scales/mean"] = torch.mean(sample_means).detach().item()
            metrics["critic/scales/max"] = torch.max(sample_means).detach().item()
            metrics["critic/scales/min"] = torch.min(sample_means).detach().item()
        else:
            metrics["critic/scales/mean"] = 0.0
            metrics["critic/scales/max"] = 0.0
            metrics["critic/scales/min"] = 0.0
    else:
        metrics["critic/scales/mean"] = 1.0
        metrics["critic/scales/max"] = 1.0
        metrics["critic/scales/min"] = 1.0
    ###
