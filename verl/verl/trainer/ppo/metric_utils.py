...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, config = None) -> dict[str, Any]:
    ...
    if "actions" in batch.batch:
        ...
        default_step = float(getattr(config, "discrete_step", min_scale))
        out = {
            "scales": batch.batch.get("scales", None),
            "actions": batch.batch.get("actions", None),
        ...
        metrics["critic/scales/mean"] = torch.mean(torch.tensor(sample_scales)).detach().item()
        metrics["critic/scales/max"] = torch.max(torch.tensor(sample_scales)).detach().item()
        metrics["critic/scales/min"] = torch.min(torch.tensor(sample_scales)).detach().item()
