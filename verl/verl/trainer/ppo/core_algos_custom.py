...
_GRPO_HADW_STATE: dict[str, float] = {}


def _apply_hadw_to_grpo_scalars(
    scalars: torch.Tensor,
    group_means: torch.Tensor,
    group_index: torch.Tensor,
    config: Optional[AlgoConfig],
) -> torch.Tensor:
    if config is None or not config.get("use_hadw_in_grpo", False):
        return scalars

    beta = float(config.get("hadw_beta", 1.0))
    ema = float(config.get("hadw_ema", 0.05))
    ema = max(0.0, min(1.0, ema))
    clip = float(config.get("hadw_clip", 3.0))
    clip = max(0.0, clip)

    with torch.no_grad():
        group_difficulty = (1.0 - group_means).clamp(0.0, 1.0)
        difficulty_mean = float(group_difficulty.mean().item())
        prev = _GRPO_HADW_STATE.get("difficulty_anchor")
        if prev is None:
            anchor = difficulty_mean
        else:
            anchor = (1.0 - ema) * float(prev) + ema * difficulty_mean
        _GRPO_HADW_STATE["difficulty_anchor"] = anchor

        anchor_t = torch.tensor(anchor, device=scalars.device, dtype=scalars.dtype)
        logw = beta * (group_difficulty[group_index].to(dtype=scalars.dtype) - anchor_t)
        if clip > 0.0:
...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        id2weight = None
        if config is not None and config.get("use_hadw_in_grpo", False):
            beta = float(config.get("hadw_beta", 1.0))
            ema = float(config.get("hadw_ema", 0.05))
            ema = max(0.0, min(1.0, ema))
            clip = float(config.get("hadw_clip", 3.0))
            ...
            id2difficulty = {k: max(0.0, min(1.0, 1.0 - float(v.item()))) for k, v in id2mean.items()}
            if len(id2difficulty) > 0:
                difficulty_mean = float(sum(id2difficulty.values()) / len(id2difficulty))
            else:
                difficulty_mean = 0.0
            prev = _GRPO_HADW_STATE.get("difficulty_anchor")
            if prev is None:
            ...
            id2weight = {}
            for k, d in id2difficulty.items():
                logw = beta * (d - anchor)
                if clip > 0.0:
                    logw = max(-clip, min(clip, logw))
                id2weight[k] = float(np.exp(logw))
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        scalars = _apply_hadw_to_grpo_scalars(scalars, mean_g, g, config)
