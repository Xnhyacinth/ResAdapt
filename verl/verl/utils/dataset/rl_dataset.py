...
from resadapt.utils.utils import tensor_to_tensor_list, tensor_to_temporal_stack_list, split_video_metadata
###
...
class RLHFDataset(Dataset):
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        max_frames = int(self.config.get("max_frames", self.config.get("MAX_FRAMES", 8)))
                        cached_key = f"len_frames_{max_frames}"
                        cached_len = doc.get(cached_key)
                        if isinstance(cached_len, (int, np.integer)):
    ...
    def _build_messages(self, example: dict, max_frames: Optional[int] = None):
        ...
        max_pixels = int(self.config.get("max_pixels", self.config.get("MAX_PIXELS", 602112)))
        max_frames = (
            int(self.config.get("max_frames", self.config.get("MAX_FRAMES", 8)))
            ...
            for segment in segments:
                if segment == "<image>":
                    ...
                    if isinstance(image, Image.Image):
                        ...
                        content_list.append({"type": "image", "image": image, "max_pixels": max_pixels})
                    elif isinstance(image, dict):
                        if "bytes" in image:
                            image["image"] = Image.open(BytesIO(image["bytes"]))
                        content_list.append({"type": "image", **image, "max_pixels": max_pixels})
                    else:
                    ...
                    ###
                    content_list.append({"type": "video", **videos[video_offset], "max_frames": max_frames, "max_pixels": max_pixels})
                    ###
                    video_offset += 1
                else:
    ...
    def doc_to_len(self, doc: dict, max_frames: Optional[int] = None) -> int:
        try:
            doc_copy = copy.deepcopy(doc)
            messages = self._build_messages(doc_copy, max_frames=max_frames)
            apply_kwargs = dict(**self.apply_chat_template_kwargs)
            if self.tool_schemas is not None:
                apply_kwargs["tools"] = self.tool_schemas

            if self.processor is None:
                return len(self.tokenizer.apply_chat_template(doc_copy[self.prompt_key], add_generation_prompt=True, **apply_kwargs))
            ...
            return len(
                self.processor(text=[raw_prompt], images=images, videos=videos, videos_kwargs=videos_kwargs)["input_ids"][
                    0
                ]
            )
        except Exception:
