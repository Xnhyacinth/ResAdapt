...
class vLLMHttpServer:
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        ###
        if _VLLM_VERSION > version.parse("0.14.1"):
    ...
    async def generate(
        ...
        ###
        if video_data is not None and "video_timestamps" in video_data[0][1].keys():
            prompt_ids = _qwen2_5_vl_dedup_video_tokens(prompt_ids, self.model_config.processor, videos=video_data)
        else:
        ###
...
def _qwen2_5_vl_dedup_video_tokens(
    prompt_ids: list[int],
    processor,
    videos = None,  # List of (video_tensor, metadata)
):
    """
    Deduplicate consecutive video tokens for Qwen2.5-VL efficiently.
    - Only processes video tokens; leaves other tokens (including images) unchanged.
    - Uses index pointers (O(1) per chunk) and advances vid_ptr across chunks once.
    - Determines how many video tokens to keep from metadata 'video_timestamps'
      grouping if present, otherwise from tensor.shape[0] and temporal_patch_size.
    """
    if processor is None or "Qwen2VLImageProcessor" not in processor.image_processor.__class__.__name__:
        return prompt_ids

    temporal_patch_size = getattr(processor.video_processor, "temporal_patch_size", 2)
    video_token_id = processor.video_token_id
    arr_ids = np.array(prompt_ids)

    mask = np.ones(len(arr_ids), dtype=bool)

    is_visual_pad = (arr_ids == processor.image_token_id) | (arr_ids == video_token_id)
    
    mask[1:] &= ~(is_visual_pad[1:] & is_visual_pad[:-1])
    
    deduped_ids = arr_ids[mask].tolist()
    ...
    video_chunk_counts = []
    if videos:
        starts = [i for i, x in enumerate(videos) if x[1].pop("video_timestamps", 0) == 0]
        starts.append(len(videos))
        video_chunk_counts = [starts[i+1] - starts[i] for i in range(len(starts)-1)]
    
    video_counts_iter = iter(video_chunk_counts)
    final_ids = []
    
    append = final_ids.append
    extend = final_ids.extend
    
    for token in deduped_ids:
        if token == video_token_id:
            count = next(video_counts_iter, 1)
            
            if count > 1:
                extend([video_token_id] * count)
            else:
                append(video_token_id)
        else:
            append(token)
