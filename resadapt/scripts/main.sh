...
MODEL_PATH=${MODEL_PATH:-"YOUR_WORKSPACE_PATH/models/qwen2.5_vl-${model_size}"}
TRAIN_FILE=${TRAIN_FILE:-"YOUR_WORKSPACE_PATH/ResAdapt/data/VisionThink-General-Train/train.parquet"}
TEST_FILE=${TEST_FILE:-"YOUR_WORKSPACE_PATH/ResAdapt/data/VisionThink-General-Val/train.parquet"}
...
if [[ "$scale_multi_modal_data" == *"nframes"* ]]; then
    ...
    if [[ "$scale_multi_modal_data" == *"sft"* ]]; then
        MODEL_PATH=YOUR_WORKSPACE_PATH/models/videor1
        # ppo_micro_batch_size_per_gpu=$((${ppo_micro_batch_size_per_gpu} / 2))
    fi

    if [[ "$scale_multi_modal_data" == *"pt"* ]]; then
        MODEL_PATH=YOUR_WORKSPACE_PATH/ResAdapt/ckpts/VideoQA_Qwen_Verify/r4-mix_base_nf64_video-dp2-7B-bsz128-mini64-n16-l8-r8/global_step_40
        # ppo_micro_batch_size_per_gpu=$((${ppo_micro_batch_size_per_gpu} / 2))
    fi

    if [[ "$scale_multi_modal_data" == *"offline"* ]]; then
        TRAIN_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-${nframes}frames/train.parquet
        TEST_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-${nframes}frames/val.parquet
    elif [[ "$scale_multi_modal_data" == *"mix"* ]]; then
        TRAIN_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/videoautor1_mixed/train_lens.parquet
        TEST_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-10K/val.parquet
        ...
        TRAIN_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/videoautor1/train.parquet
        TEST_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-10K/val.parquet
        total_training_steps=600
    elif [[ "$scale_multi_modal_data" == *"r1"* ]]; then
        TRAIN_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/videor1/train.parquet
        TEST_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-10K/val.parquet
        total_training_steps=800
    else
        TRAIN_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-10K/train.parquet
        TEST_FILE=YOUR_WORKSPACE_PATH/ResAdapt/data/TSPO-10K/val.parquet
    fi
    ...
    # extra_args+=" data.custom_cls.path=YOUR_WORKSPACE_PATH/ResAdapt/resadapt/allocator/rl_dataset.py"
...
PROJECT_DIR="$(pwd)"
CONFIG_PATH="$PROJECT_DIR/resadapt/config"
CKPTS_DIR=${CKPTS_DIR:-"${RAY_DATA_HOME}/ckpts/${project_name}/${exp_name}"}

# ray job submit --no-wait --runtime-env="${RUNTIME_ENV}" \
#     --address "${RAY_ADDRESS}" \
#     --working-dir "${WORKING_DIR}" \
    # -- python3 -m verl.resadapt.main_ppo \
        # +actor_rollout_ref.model.override_config.max_position_embeddings=32768 \
cmd=(
    python3 -m resadapt.main_ppo
    ...
    custom_reward_function.path=YOUR_WORKSPACE_PATH/ResAdapt/resadapt/reward_fn/reward_r1.py
