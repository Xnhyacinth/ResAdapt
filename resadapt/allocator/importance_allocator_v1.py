...
class FrameInformationEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Stack and project
        raw_stats = torch.stack([patch_var.float(), norm_var, concentration, sparsity, peak], dim=-1)  # (B, T, 5)
    ...
    def forward(
        ...
        if _SDPA_AVAILABLE:
            ...
            if text_mask is not None:
                # Create additive mask efficiently: True->0, False->-inf
                # Using where instead of zeros_like + masked_fill to avoid extra allocation
                mask_bool = text_mask.unsqueeze(1).unsqueeze(2)  # (B, 1, 1, N)
                attn_mask = torch.where(
                    mask_bool,
                    torch.zeros(1, dtype=q.dtype, device=q.device),
                    torch.tensor(float('-inf'), dtype=q.dtype, device=q.device)
                ).expand(B, 1, T, N)  # expand is a view, no copy
...
class SparseGatingModule(nn.Module):
    ...
    def forward(
        ...
        if self.mode == "gumbel_softmax":
            ...
            gate = F.softmax(gate_logits / tau_float, dim=-1)
...
class DualPathEncoder(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int = 4, dropout: float = 0.0):
        ...
        # Per-dimension fusion gate (avoid over-smoothing affecting all channels equally)
        # Outputs (B, T, D) gate value
        self.fusion_gate = nn.Sequential(
            nn.Linear(dim * 2, dim // 2),
            nn.GELU(),
            nn.Linear(dim // 2, dim),  # Per-dimension gate
        )
    ...
    def forward(
        ...
        temporal, _ = self.temporal_attn(x_norm, x_norm, x_norm, key_padding_mask=key_padding_mask)
        temporal = x + self.temporal_out_dropout(temporal)  # Residual with dropout
        temporal = temporal + self.temporal_out_dropout(self.temporal_ff(temporal))
        
        # Fusion with per-frame SCALAR gate (biased towards independent)
        concat = torch.cat([independent, temporal], dim=-1)
        gate = torch.sigmoid(self.fusion_gate(concat))  # (B, T, 1)
...
class DifferentiableImportanceAllocator(ModuleUtilsMixin, nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # 1. Frame Information Encoder
        self.info_encoder = FrameInformationEncoder(dim=dim, info_dim=8)
        ...
        # 4. Dual-Path Encoder
        self.dual_encoder = DualPathEncoder(dim=dim, num_heads=max(1, heads // 2), dropout=dropout)
        
        # 5. Contrastive Differentiator
        self.contrastive = ContrastiveDifferentiator(dim=dim, temperature=contrastive_temperature)
    ...
    def _get_continuous_policy(self, params, actions=None):
        ...
        if self.continuous_dist == "beta":
            ...
            if actions is None:
                use_reparam_action = (self.sim_scale_weight > 0) or (getattr(self, "contrastive_weight", 0.0) > 0)
    ...
    def _get_continuous_deterministic(self, params):
        ...
        if self.continuous_dist == "beta":
            ...
            if self.continuous_eval_quantile != 0.5:
                q = torch.tensor(self.continuous_eval_quantile, device=params.device).clamp(1e-4, 1-1e-4)
            ...
            if self.continuous_eval_quantile != 0.5:
                q = torch.tensor(self.continuous_eval_quantile, device=params.device).clamp(1e-4, 1-1e-4)
                z = Normal(0.0, 1.0).icdf(q).to(mu.dtype)
                action_0_1 = torch.sigmoid(mu + sigma * z)
            else:
                denom = torch.sqrt(1.0 + (torch.pi * sigma * sigma / 8.0))
                action_0_1 = torch.sigmoid(mu / denom)
    ...
    def forward(
        ...
        for group_idx, tl in enumerate(unique_pairs):
            ...
            # ===== Step 2: Initial frame pooling for cross-modal matching =====
            frame_mean = feats.mean(dim=2)  # (Bobj, T, D)
            
            # ===== Step 3: Cross-Modal Matching =====
            if enc_states is not None:
                relevance, text_attended = self.cross_matcher(frame_mean, enc_states, enc_mask)
                gate_query = self.gate_query_proj(text_attended)
            else:
                relevance = torch.zeros(Bobj, t, device=device, dtype=feats.dtype)
                gate_query = frame_mean
            
            # ===== Step 4: Sparse Gating for patch pooling =====
            # FIX: Use soft gating (hard=False) even in eval mode to prevent mode collapse.
            # Hard gating (one-hot) during eval creates drastic distribution shift from soft training.
            ...
            # ===== Step 6: Contrastive Loss (training only) =====
            if self.training and self.contrastive_weight > 0:
                c_loss = self.contrastive(independent)
                contrastive_loss_sum += c_loss
                contrastive_count += 1
            # elif idx == 0:  # Debug: print only once per forward
            #     print(f"[DIP-debug] skipping contrastive: training={self.training}, contrastive_weight={self.contrastive_weight}")
            ...
            frame_features = self.feature_fuser(combined)  # (Bobj, T, D)
            
            # ===== Step 8: Scale Prediction =====
            scale_params = self.scale_head(frame_features)  # (Bobj, T, output_dim)
            
            # Store outputs
            head_outputs[idx, :t] = scale_params
            frame_features_out[idx, :t] = frame_features
            
            # ===== Compute Frame Metrics (scalar, memory-efficient) =====
            if compute_frame_metrics:
                with torch.no_grad():
                    ...
                    if t > 1:
                        ...
                        sim_prev = (ff_norm[:, 1:] * ff_norm[:, :-1]).sum(dim=-1)  # (Bobj, T-1)
                        # Combine prev and next similarities
                        sim_prev_padded = F.pad(sim_prev, (1, 0), value=0.0)  # (Bobj, T)
            ...
            # Debug stats
            if eval_mode:
                gate_ent = -(gate.clamp(1e-9) * gate.clamp(1e-9).log()).sum(dim=-1).mean()
                gate_entropy_sum += gate_ent.item()
                if t > 1:
                    f_sim = F.cosine_similarity(frame_features[:, :-1], frame_features[:, 1:], dim=-1).mean()
        ...
        if self.sim_scale_weight > 0 and self.training and frame_features_out.shape[1] > 1:
            ...
            # print(f"[DIP-debug] sim_scale_loss computed: {self._last_sim_scale_loss.item():.6f}, requires_grad={self._last_sim_scale_loss.requires_grad}")
        else:
            self._last_sim_scale_loss = torch.tensor(0.0, device=device)
            # print(f"[DIP-debug] skipping sim_scale: weight={self.sim_scale_weight}, training={self.training}, T={frame_features_out.shape[1]}")
        ...
        if eval_mode and debug_count > 0:
            ...
            print(
                f"[DIP-debug] gate_entropy={gate_entropy_sum/debug_count:.4f} "
                f"frame_sim={frame_sim_sum/debug_count:.4f} "
                f"scale_var={var.mean().item():.6f} "
                f"contrastive_loss={c_loss_val:.4f}"
            )
