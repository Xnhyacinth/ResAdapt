...
###
from resadapt.utils.utils import to_numpy_cpu
...
class DataParallelPPOActor(BasePPOActor):
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                ...
                if compute_frame_metrics:
                    frame_metrics = scaled_inputs.get("frame_metrics")
                    if isinstance(frame_metrics, dict):
                        tensor_updates.update(encode_frame_metrics(frame_metrics))
    ...
    def collect_allocator_log_probs(self, data: DataProto, *, video2list: bool = False, video2image: bool = False):
        ...
        self.actor_module.eval()
        try:
            eval_batch_keys = ["actions", "scale_mask"]
            if self.use_dynamic_bsz and "input_ids" in data.batch:
                eval_batch_keys.append("input_ids")
            eval_data = data.select(
                batch_keys=eval_batch_keys,
                non_tensor_batch_keys=["ori_prompt"],
            )
            if self.use_dynamic_bsz and "input_ids" in eval_data.batch:
                max_token_len = self.config.ppo_max_token_len_per_gpu * self.ulysses_sequence_parallel_size
                micro_batches, _ = prepare_dynamic_batch(eval_data, max_token_len=max_token_len)
            else:
