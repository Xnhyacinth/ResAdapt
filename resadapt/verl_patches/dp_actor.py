                ...
                for micro_batch in micro_batches:
                    ...
                    if is_pred and "allocator_log_probs" in model_inputs.keys():
                        ...
                        if scale_one_mask is not None:
                            ...
                            if negative_approx_kl.dim() > 1:
                                scale_one_mask_tensor = scale_one_mask_tensor.unsqueeze(1).expand_as(negative_approx_kl)
                            negative_approx_kl = negative_approx_kl.masked_fill(scale_one_mask_tensor, 0.0)
                        if scale_mask is not None:
                            negative_approx_kl = negative_approx_kl * scale_mask.float()
                        negative_approx_kl = negative_approx_kl.sum(dim=-1, keepdim=True)

                        # Clamp negative_approx_kl for stability
                        negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
                        ratio = torch.exp(negative_approx_kl).expand(-1, advantages.shape[-1])
