...
from visionthink.reward_fn.reward_r1 import compute_score
...
def _build_ee_prompt(ex: Dict[str, Any]) -> str:
    ...
    problem_type = str(ex.get("problem_type") or "")
    suffix = type_template.get(problem_type, " Please provide your text answer within the <answer> </answer> tags.")
    prompt = question_template.format(Question=question) + suffix
...
def _video_to_images(video_path: str, num_frames: int) -> List["Image.Image"]:
...
def _apply_scales_to_images(images: List["Image.Image"], scales: List[float]) -> List["Image.Image"]:
    from PIL import Image
    scaled: List[Image.Image] = []
    n = min(len(images), len(scales))
    for i in range(n):
        s = max(0.2, min(2.0, float(scales[i])))
        img = images[i]
        new_w = max(1, int(img.width * s))
        new_h = max(1, int(img.height * s))
        scaled.append(img.resize((new_w, new_h), resample=Image.BICUBIC))
    for i in range(n, len(images)):
        scaled.append(images[i])
    return scaled
...
def _normalize_ans(ans: Optional[str]) -> Optional[str]:
    if ans is None:
        return None
    return re.sub(r"\\s+", " ", ans.strip()).upper()
...
async def _call_openai_for_answer(
    client,
    *,
    model: str,
    system_message: str,
    ...
    *,
    ...
    if data_type == "image":
        try:
            ...
            from PIL import Image
            original_images = [Image.open(data_path).convert("RGB")]
        except Exception:
            media_url = f"file://{data_path}"
    else:
        original_images = _video_to_images(data_path, num_frames)
        if not original_images:
            ...
            async with counter_lock:
                ...
                pbar.update(1)
            return None
        media_urls = [_encode_image_to_base64(img) for img in original_images]
    ...
    gt_answer_raw = ex.get("solution") or ""
    system_msg_for_answer, answer_prompt = _build_ee_prompt(ex)
    success = False
    final_scales = scales

    for attempt in range(max_scale_attempts):
        if data_type == "image":
            scaled_imgs = _apply_scales_to_images(original_images, final_scales)
            scaled_urls = [_encode_image_to_base64(scaled_imgs[0])]
        else:
            scaled_imgs = _apply_scales_to_images(original_images, final_scales)
            scaled_urls = [_encode_image_to_base64(img) for img in scaled_imgs]

        try:
            answer_text = await _call_openai_for_answer(
                client,
                model=model,
                system_message=system_msg_for_answer,
                question=answer_prompt,
                data_type=data_type,
                media_urls=scaled_urls,
                max_tokens=max_tokens,
            )
        except Exception as e:
            await _write_fail({"id": record_id, "path": data_path, "data_type": data_type, "reason": "answer_call_failed", "error": repr(e), "attempt": attempt + 1})
            continue

        score = compute_score(
            solution_str=answer_text,
            ground_truth=gt_answer_raw,
            extra_info={"problem_type": ex.get("problem_type")},
        )
        if score.get("acc_reward", 0.0) > 0.5:
            success = True
            break

        try:
            final_scales = await _call_openai_for_scales(
                client,
                model=model,
                question=question_text,
                data_type=data_type,
            ...
            await _write_fail({"id": record_id, "path": data_path, "data_type": data_type, "reason": "retry_scales_failed", "error": repr(e), "attempt": attempt + 1})
            break
    ...
    record = {
        ...
        "scales": final_scales if success else [],
    ...
    async with write_lock:
        if record_id in done_ids:
            async with counter_lock:
                ...
                pbar.update(1)
            return None
        if not success or not record["scales"]:
            await _write_fail({"id": record_id, "path": data_path, "data_type": data_type, "reason": "empty_scales_or_answer_mismatch"})
            async with counter_lock:
                counters["failed"] = counters.get("failed", 0) + 1
                pbar.update(1)
            return None
