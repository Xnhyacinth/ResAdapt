...
class RayPPOTrainer:
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        if self._predictor_scale_enabled():
            reward_model_keys.update(["ori_prompt", "sid"])
        ###
    ...
    def _generate_sequences(self, gen_batch_padded: "DataProto") -> "DataProto":
        if not self.async_rollout_mode:
            return self.actor_rollout_wg.generate_sequences(gen_batch_padded)
    ...
    def _run_validation_pass(
        ...
        *,
        ...
        reward_tensor, reward_extra_info = self._compute_or_extract_reward(
            merged_batch, reward_fn=reward_fn, reward_for_val=True
        )
    ...
    def _scale_batch_with_predictor(
        ...
        size_divisor = (
            self.actor_rollout_wg.world_size
            if not self.async_rollout_mode
            else self.config.predictor.scale.num_workers
        )
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            if self.config.reward_model.enable:
                style = test_batch[0].non_tensor_batch.get("reward_model", {}).get("style", None)
                if style == "model":
                    return {}
    ...
    def _val_metrics_update(
        ...
        *,
        ...
        if scaled_payload is not None:
            ...
            scaled_var2metrics = process_validation_metrics(ds_scaled, sample_uids, extra_scaled)
