...
###
def get_vl_model_embed_tokens(vl_model_instance):
    if hasattr(vl_model_instance, "model") and hasattr(vl_model_instance.model, "language_model"):
        language_model = vl_model_instance.model.language_model
        if hasattr(language_model, "embed_tokens"):
            return language_model.embed_tokens
    if hasattr(vl_model_instance, "language_model") and hasattr(vl_model_instance.language_model, "embed_tokens"):
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if self._is_actor:
            ...
            ###
            self.config.actor.ppo_mini_batch_size *= self.config.scale_n
            ###
    ...
    def _build_model_optimizer(
        ...
        ###
        self.tokenizer = hf_tokenizer(local_path, trust_remote_code=trust_remote_code, fix_mistral_regex=True)
        ###
    ...
    def init_model(self):
        ...
        ###
        from visionthink.predictor.dp_actor import DataParallelPPOActor
        ###
...
class PredictorWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        actor_model_config._attn_implementation = "flash_attention_2"
    ...
    def init_model(self):
        # from verl.workers.actor import DataParallelPPOActor
        ###
        from visionthink.predictor.dp_predictor import DataParallelPPOActor
        ###
    ...
    def update_predictor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                ###
                data.meta_info["video2list"] = self.config.get("video2list", False)
                ...
                data.meta_info["sim_scale_coef"] = self.config.scale.get("sim_scale_coef", 0.0)
                ...
                if scale_multi_modal_data and "aw" in scale_multi_modal_data:
            ...
            tensors = {}
            res_idx = 1
            if is_pred and len(res) > res_idx:
                predictor_log_probs = res[res_idx]
                tensors["predictor_log_probs"] = predictor_log_probs
                res_idx += 1
            if compute_frame_metrics and len(res) > res_idx:
                frame_metrics = res[res_idx]
                if frame_metrics is not None and isinstance(frame_metrics, dict):
                    for k, v in frame_metrics.items():
                        if isinstance(v, torch.Tensor):
                            tensors[f"frame_metrics:{k}"] = v
            if tensors:
                output = DataProto.from_dict(tensors=tensors, meta_info={"metrics": metrics})
            else:
                output = DataProto(meta_info={"metrics": metrics})
            ###
