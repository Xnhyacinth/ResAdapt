...
class HeadOnlyPredictor(ModuleUtilsMixin, nn.Module):
    ...
    def post_init(self):
        ...
        if head.weight is not None:
            nn.init.normal_(head.weight, mean=0.0, std=self.init_weight_std)
        scale = max(self.beta_param_scale, 1e-6)
    ...
    def forward(
        ...
        head_outputs_obj = torch.zeros((B, max_t_obj, self.regression_head[-1].out_features), dtype=dtype, device=device)
        ...
        splits = torch.split(object_t_dims, [1] * B)
        ...
        def _restructure(tensor, pad_val):
            if tensor is None:
                return None
            ...
            output = torch.full(scale_mask_out.shape, pad_val, dtype=tensor.dtype, device=device)
            output[scale_mask_out] = valid_values
            return output

        if compute_frame_metrics:
            self._last_frame_metrics = {
                "redundancy": _restructure(redundancy_obj, 0.0),
                "uniqueness": _restructure(uniqueness_obj, 0.5),
                "text_relevance": _restructure(text_relevance_obj, 0.0),
                "info_score": _restructure(info_score_obj, 0.0),
            }

        raw_actions = _restructure(raw_actions_obj, pad_val_action)
        scales = _restructure(scales_obj, 1.0)
        log_probs = _restructure(log_probs_obj, 0.0) if log_probs_obj is not None else None
