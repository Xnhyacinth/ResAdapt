...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            if config and "trainer" in config:
                resume_mode = config['trainer'].get("wandb_resume", None)
                run_id = config['trainer'].get("wandb_run_id", None)

            if resume_mode is None:
                resume_mode = os.environ.get("WANDB_RESUME", None)

            if run_id is None:
                run_id = os.environ.get("WANDB_RUN_ID", None)

            if resume_mode is None and run_id is not None:
                resume_mode = "allow"
...
class ValidationGenerationsLogger:
    ...
    def log(self, loggers, samples, step, column_names=None):
        if "wandb" in loggers:
            self.log_generations_to_wandb(samples, step, column_names=column_names)
    ...
    def log_generations_to_wandb(self, samples, step, column_names=None):
        import wandb

        self._log_generations_to_wandb(samples, step, wandb, column_names=column_names)

    def _log_generations_to_wandb(self, samples, step, wandb, column_names=None):
        ...
        columns = ["step"] + sum(
            [[f"{col}_{i + 1}" for col in column_names] for i in range(len(samples))], []
        )
        ###
