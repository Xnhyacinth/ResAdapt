...
# project_name='debug'
exp_name="${scale_multi_modal_data}-${scale_n}-3B-Single-bsz${train_prompt_bsz}-mini${train_prompt_mini_bsz}-n${n_resp_per_prompt}-min${min_scale}-max${max_scale}-len${max_prompt_length}"
...
if [[ "$scale_multi_modal_data" == *"flash"* ]]; then
    PREDICTOR_PATH=/mnt/bn/jiangzhongtao/users/liaohuanxuan/models/predictor_flash
else
...
python3 -m visionthink.main_ppo \
    ...
    predictor.model.path="${PREDICTOR_PATH}" \
    predictor.model.scale_multi_modal_data=${scale_multi_modal_data} \
    predictor.model.scale_n=${scale_n} \
    +predictor.model.override_config.attn_implementation=${attn_implementation} \
    +predictor.model.override_config.min_scale=${min_scale} \
    +predictor.model.override_config.max_scale=${max_scale} \
    +predictor.model.override_config.self_depth=${self_depth} \
    +predictor.model.override_config.cross_depth=${cross_depth} \
    +predictor.model.override_config.use_text=${use_text} \
