...
###
from visionthink.ray_trainer import RayPPOTrainer
###
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            ...
            ###
            from visionthink.predictor.fsdp_workers import AsyncActorRolloutRefWorker
            ###
    ...
    def init_resource_pool_mgr(self, config):
        ...
        # optional: dedicate a separate predictor pool
        if hasattr(config, "predictor") and bool(config.predictor.get("enable", False)):
            enable_pred_pool = bool(config.predictor.get("enable_resource_pool", False)) or bool(
                config.predictor.get("dedicated_8gpu", False)
            )
            if enable_pred_pool:
                if bool(config.predictor.get("dedicated_8gpu", False)):
                    predictor_pool = [8] * 1
                else:
                    n_pred_gpus = int(config.predictor.get("n_gpus_per_node", 0))
                    n_pred_nodes = int(config.predictor.get("nnodes", 0))
                    if n_pred_gpus <= 0:
                        raise ValueError("config.predictor.n_gpus_per_node must be greater than 0")
                    if n_pred_nodes <= 0:
                        raise ValueError("config.predictor.nnodes must be greater than 0")
                    predictor_pool = [n_pred_gpus] * n_pred_nodes
                resource_pool_spec["predictor_pool"] = predictor_pool
    ...
    def add_predictor_worker(self, config):
        ...
        if config.predictor.actor.strategy in {"fsdp", "fsdp2"}:
            from visionthink.predictor.fsdp_workers import AsyncPredictorWorker
        elif config.predictor.actor.strategy == "megatron":
            from visionthink.predictor.megatron_workers import AsyncPredictorWorker
        else:
    ...
    def run(self, config):
        ...
        ###
        val_data_config = OmegaConf.merge(config.data, OmegaConf.create({"max_frames": 16}))
