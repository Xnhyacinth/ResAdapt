...
class PredictorForConditionalGeneration(PreTrainedModel):
    ...
    def __init__(self, config: PredictorConfig):
        ...
        print(f"set predictor max_frames: {vc.max_frames}")
        ...
        if predictor_version == "v2":
            from visionthink.predictor.AZNetv3 import RegressionHeadPredictorV2
    ...
    def scale_multi_modal(
        ...
        **kwargs,
        ...
        if actions is not None:
            ...
            scale_var = getattr(self.predictor, "_last_scale_var", None)
            if scale_var is None and hasattr(self.predictor, "scorer"):
                scale_var = getattr(self.predictor.scorer, "_last_scale_var", None)
            concentration_loss = getattr(self.predictor, "_last_concentration_loss", None)
            if concentration_loss is None and hasattr(self.predictor, "scorer"):
                concentration_loss = getattr(self.predictor.scorer, "_last_concentration_loss", None)
            ...
            if not compute_aux:
                ...
                if frame_metrics is None:
                    frame_metrics = {}
                entropy = None
                if hasattr(self.predictor, "scorer"):
...
def main():
    ...
    model_path = "/mnt/bn/jiangzhongtao/users/liaohuanxuan/models/predictorv3_beta_deep_zero"
