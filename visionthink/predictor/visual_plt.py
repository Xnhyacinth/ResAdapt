...
from visionthink.predictor.AZNet0 import RegressionHeadPredictor
...
class UniQwenVisionTransformer(Qwen2_5_VisionTransformerPretrainedModel):
    ...
    def forward(self, pixel_values, grid_thw, output_hidden_states=None, return_dict=None, **kwargs):
...
class PredictorForConditionalGeneration(PreTrainedModel):
    ...
    def scale_multi_modal(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            try:
                if hasattr(self.vision_tower, "config") and hasattr(self.vision_tower.config, "output_hidden_states"):
                    self.vision_tower.config.output_hidden_states = True
            except Exception:
                pass
            def _collect_hidden_states(pixel_values_in, grid_thw_in):
                if pixel_values_in is None or grid_thw_in is None:
                    print("None pixel_values_in or grid_thw_in")
                    return None
                try:
                    outputs = self.vision_tower(pixel_values_in, grid_thw_in, output_hidden_states=True, return_dict=True)
                except Exception:
                    try:
                        outputs = self.vision_tower(pixel_values_in, grid_thw_in)
                    except Exception:
                        print("None outputs")
                        return None
            ...
            def _concat_hidden_states(a, b):
                if a is None:
                    return b
                if b is None:
                    return a
                return [torch.cat([x, y], dim=0) for x, y in zip(a, b)]

            def _compute_layer_stats(hidden_states, grid_thw):
                sim_means = []
                var_means = []
                split_sizes = (grid_thw.prod(-1) // merge_len).tolist()
                for feats in hidden_states:
                    if feats.ndim == 3:
                        feats = feats.reshape(-1, feats.shape[-1])
                    chunks = torch.split(feats, split_sizes)
                    sim_sum = 0.0
                    sim_count = 0
                    var_sum = 0.0
                    var_count = 0
                    for i, chunk in enumerate(chunks):
                        ...
                        l = int((grid_thw[i, 1].item() * grid_thw[i, 2].item()) // merge_len)
                        if t <= 0 or l <= 0:
                            continue
                        cur = chunk[: t * l].reshape(t, l, -1)
                        mean = cur.mean(dim=1)
                        if t > 1:
                            sim = F.cosine_similarity(mean[:-1], mean[1:], dim=-1)
                            sim_sum += sim.mean().item()
                            sim_count += 1
                        norm = mean.float().norm(dim=-1)
                        var_sum += norm.var(unbiased=False).item()
                        var_count += 1
                    sim_means.append(sim_sum / max(1, sim_count))
                    var_means.append(var_sum / max(1, var_count))
                return sim_means, var_means

            hidden_states = None
            grid_thw = None
            if pixel_values is not None and image_grid_thw is not None:
                print("images")
                hs_img = _collect_hidden_states(pixel_values, image_grid_thw)
                hidden_states = _concat_hidden_states(hidden_states, hs_img)
                grid_thw = image_grid_thw
            if pixel_values_videos is not None and video_grid_thw is not None:
                print("videos")
                hs_vid = _collect_hidden_states(pixel_values_videos, video_grid_thw)
                hidden_states = _concat_hidden_states(hidden_states, hs_vid)
                grid_thw = video_grid_thw if grid_thw is None else torch.cat([grid_thw, video_grid_thw], dim=0)

            if hidden_states is None or grid_thw is None:
                raise RuntimeError("vision_tower hidden_states not available for per-layer plot")
            sim_means, var_means = _compute_layer_stats(hidden_states, grid_thw)
            os.makedirs("figs", exist_ok=True)
            layers = list(range(len(var_means)))
            plt.figure(figsize=(10, 5))
            ax1 = plt.gca()
            ax1.plot(layers, var_means, color="tab:blue")
            ax1.set_xlabel("layer")
            ax1.set_ylabel("variance", color="tab:blue")
            ax1.tick_params(axis="y", labelcolor="tab:blue")
            ax2 = ax1.twinx()
            ax2.plot(layers, sim_means, color="tab:red")
            ax2.set_ylabel("similarity", color="tab:red")
            ax2.tick_params(axis="y", labelcolor="tab:red")
            plt.title("vision_tower hidden states: similarity and variance per layer")
            plt.tight_layout()
            plt.savefig(os.path.join("figs", "vision_tower_sim_var.png"))
            plt.close()
        ...
        if actions is not None:
            ...
            scales = getattr(self.predictor, "last_scales", None)
            return {
                "log_probs": log_probs,
