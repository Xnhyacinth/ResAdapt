...
class FrameWiseScalePredictor(ModuleUtilsMixin, nn.Module):
    ...
    def __init__(
        ...
        beta_param_scale=1.0, beta_add_one=True,
        ...
        with torch.no_grad():
            if self.use_discrete_action:
                ...
                if self.continuous_dist == "logistic_normal":
                    ...
                    # FIX: Use smaller target_sum (2.0 instead of 4.0) for higher initial entropy
                    # This makes initial Beta distribution more uniform (alpha=0.5, beta=0.5)
                    # allowing more exploration across the full [0,1] range
    ...
    def _get_continuous_policy(self, params, actions=None):
        ...
        if self.continuous_dist == "beta":
            ...
            dist0 = Beta(alpha.contiguous(), beta.contiguous())
            if actions is None:
            ...
            log_prob = dist0.log_prob(action_0_1)
        else:
    ...
    def forward(
        ...
        if self.sim_scale_weight > 0 and self.training and frame_features_obj.shape[1] > 1:
...
class RegressionHeadPredictorV2(ModuleUtilsMixin, nn.Module):
    ...
    def __init__(self, vision_config):
        super().__init__()
        from visionthink.predictor.DifferentiableImportancePredictor import DifferentiableImportancePredictor
