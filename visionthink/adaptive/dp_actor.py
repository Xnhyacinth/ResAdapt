...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None, predictor_optimizer = None):
    ...
    def scale_multi_modal(self, data: DataProto, eval_mode=False):
        ...
        non_tensor_batch_keys_to_pop = ["multi_modal_inputs"]
        updated_data.pop(
            # batch_keys=batch_keys_to_pop,
            non_tensor_batch_keys=list(non_tensor_batch_keys_to_pop),
        )

        # updated_data.batch["copy_input_ids"] = updated_data.batch["input_ids"].clone()
        # updated_data.batch["copy_attention_mask"] = updated_data.batch["attention_mask"].clone()
        # updated_data.batch["copy_position_ids"] = updated_data.batch["position_ids"].clone()
        # updated_data.non_tensor_batch["copy_multi_modal_inputs"] = updated_data.non_tensor_batch["multi_modal_inputs"].copy()
        # updated_data.non_tensor_batch["copy_multi_modal_data"] = updated_data.non_tensor_batch["multi_modal_data"].copy()
        # if not eval_mode:
        #     breakpoint()
    ...
    def update_policy(self, data: DataProto):
        ...
        # if self.config.get("scale_multi_modal_data", None):
            select_keys.extend(["ori_input_ids", "ori_attention_mask", "ori_position_ids", "actions", "predictor_old_log_probs", "scales", "scale_mask"])
            non_tensor_select_keys.extend(["ori_multi_modal_inputs", "ori_multi_modal_data"])

            # if self.config.get("scale_n", 1) > 1:
            if "predictor_advantages" in data.batch.keys():
                select_keys.extend(["predictor_advantages"])
        ###
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if hasattr(self.config, "use_rollout_log_probs") and self.config.use_rollout_log_probs:
                        old_log_prob = model_inputs["old_log_probs"]
                        ###
                        predictor_old_log_prob = model_inputs.get("predictor_old_log_probs", None)
                        ###
                    else:
                        if on_policy:
                            old_log_prob = log_prob.detach()
                            ###
                            predictor_old_log_prob = predictor_log_prob.detach()
                            ###
                        else:
                            old_log_prob = model_inputs["old_log_probs"]
                            ###
                    ...
                    predictor_advantages = model_inputs.get("predictor_advantages", None)

                    predictor_log_prob = self.compute_pred_log_prob(model_inputs)

                    if is_pred and predictor_log_prob is not None:
                        pred_negative_approx_kl = predictor_log_prob - predictor_old_log_prob
                        if scale_mask is not None:
                            pred_negative_approx_kl = pred_negative_approx_kl * scale_mask.float()
                        pred_negative_approx_kl = pred_negative_approx_kl.sum(dim=-1, keepdim=True)

                        # Clamp pred_negative_approx_kl for stability
                        pred_negative_approx_kl = torch.clamp(pred_negative_approx_kl, min=-20.0, max=20.0)
                        pred_ratio = torch.exp(pred_negative_approx_kl).repeat(1, advantages.shape[-1])
                        advantages = advantages * pred_ratio

                    if is_pred and predictor_advantages is not None:
                        print("is_pred")
                        negative_approx_kl = log_prob - old_log_prob
                        if response_mask is not None:
                            negative_approx_kl = negative_approx_kl * response_mask.float()
                        negative_approx_kl = negative_approx_kl.sum(dim=-1, keepdim=True)

                        # Clamp negative_approx_kl for stability
                        negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
                        ratio = torch.exp(negative_approx_kl).repeat(-1, predictor_advantages.shape[-1])
                        predictor_advantages = predictor_advantages * ratio

                    # Compute policy loss (any function is expected to return 2 values)
                    pred_pg_loss, pred_pg_metrics = policy_loss_fn(
                        old_log_prob=predictor_old_log_prob,
                        log_prob=predictor_log_prob,
                        advantages=predictor_advantages,
                        response_mask=scale_mask,
                    ...
                    for k, v in pred_pg_metrics.items():
                        if k.startswith("actor/"):
                            ...
                            pred_metrics[new_key] = v
                        else:
                            pred_metrics[f"predictor_{k}"] = v

                    micro_batch_metrics.update(pred_metrics)
