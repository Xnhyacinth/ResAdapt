    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            ###
            predictor_optimizer=kwargs.get("predictor_optimizer", None),
            predictor_lr_scheduler=kwargs.get("predictor_lr_scheduler", None),
            ###
        )
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            ...
            if self.should_load_optimizer:
                ...
                if self.predictor_optimizer is not None:
                    remote_predictor_optim_path = os.path.join(
                        local_path, f"predictor_optim_world_size_{self.world_size}_rank_{self.rank}.pt"
                    )
                    local_predictor_optim_path = copy_to_local(remote_predictor_optim_path)
                    predictor_optimizer_state_dict = torch.load(local_predictor_optim_path, weights_only=False)
                    self.predictor_optimizer.load_state_dict(predictor_optimizer_state_dict)
                    log_with_rank(f"Loaded predictor_optimizer from {remote_predictor_optim_path}", rank=self.rank, logger=logger)
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_optimizer:
                    ...
                    if self.predictor_optimizer is not None:
                        predictor_optim_path = os.path.join(
                            local_path, f"predictor_optim_world_size_{self.world_size}_rank_{self.rank}.pt"
                        )
                        predictor_optimizer_state_dict = self.predictor_optimizer.state_dict()
                        torch.save(predictor_optimizer_state_dict, predictor_optim_path)
                        log_with_rank(f"Saved predictor_optim to {os.path.abspath(predictor_optim_path)}", rank=self.rank, logger=logger)
        ...
        if self.rank == 0:
            ...
            for k in cfg_dict.keys():
                if not isinstance(k, str):
                    print("Non-string key found:", repr(k))
                if cfg_dict[k] is not None:
                    filtered_cfg_dict[k] = cfg_dict[k]
            model_config = model_config.__class__.from_dict(filtered_cfg_dict)
            # print("model_config after", model_config)
            ###
