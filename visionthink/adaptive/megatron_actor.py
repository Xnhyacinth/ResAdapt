...
class MegatronPPOActor(BasePPOActor):
    ...
    ###
    @GPUMemoryLogger(role="megatron_actor", logger=logger)
    def scale_multi_modal(self, data: DataProto, calculate_entropy=False):
        """
        Megatron-compatible implementation of scale_multi_modal.
        Handles Pipeline Parallelism (PP) by collecting outputs on the last stage 
        and broadcasting to all other stages.
        """
        import numpy as np
        import itertools
        from verl.utils.megatron import mpu
        from verl.workers.actor.megatron.optimizer import get_device_id
        from verl.utils.dataset.rl_utils import get_reverse_idx
        
        use_dynamic_bsz = data.meta_info.get("use_dynamic_bsz", False)
        micro_batch_size = data.meta_info.get("micro_batch_size", 8)
        max_token_len = data.meta_info.get("max_token_len", None)
        
        if use_dynamic_bsz:
            assert max_token_len is not None, "max_token_len must be set when use_dynamic_bsz is True"
            max_token_len = max_token_len * self.config.megatron.get("context_parallel_size", 1)
        else:
            assert micro_batch_size is not None, "micro_batch_size is needed when use_dynamic_bsz is False"
        ...
        original_data_backup = {
            ...
            "ori_multi_modal_inputs": data.non_tensor_batch["multi_modal_inputs"], 
            "ori_multi_modal_data": data.non_tensor_batch["multi_modal_data"],     
        }

        def compute_scaling_output_fn(model_output, data_batch, use_dynamic_bsz=False, indices=None):
            log_probs = model_output.get("log_probs", None)
            new_input_ids = model_output.get("input_ids", None)
            new_attention_mask = model_output.get("attention_mask", None)
            new_position_ids = model_output.get("position_ids", None)
            actions = model_output.get("actions", None)
            scales = model_output.get("scales", None)

            new_multi_modal_inputs = model_output.get("multi_modal_inputs", None)
            new_multi_modal_data = model_output.get("multi_modal_data", None)

            result = {
                "predictor_old_log_probs": log_probs,
                "input_ids": new_input_ids,
                "attention_mask": new_attention_mask,
                "position_ids": new_position_ids,
                "actions": actions,
                "scales": scales,
                "multi_modal_inputs": new_multi_modal_inputs,
                "multi_modal_data": new_multi_modal_data
            }
            return result

        with torch.no_grad():
            output_wrapper = self.forward_backward_batch(
                data,
                forward_only=True, 
                post_process_fn=compute_scaling_output_fn,
        ...
        final_results_container = [None] 

        if mpu.is_pipeline_last_stage(ignore_virtual=True):
            micro_batch_outputs = output_wrapper["output"]
            
            tensor_keys = ["predictor_old_log_probs", "input_ids", "attention_mask", "position_ids", "actions", "scales"]
            aggregated_tensors = {k: [] for k in tensor_keys}
            
            non_tensor_keys = ["multi_modal_inputs", "multi_modal_data"]
            aggregated_non_tensors = {k: [] for k in non_tensor_keys}

            for mb_out in micro_batch_outputs:
                for k in tensor_keys:
                    if mb_out[k] is not None:
                        aggregated_tensors[k].append(mb_out[k])
                
                for k in non_tensor_keys:
                    if mb_out[k] is not None:
                        if isinstance(mb_out[k], (list, tuple)):
                            aggregated_non_tensors[k].extend(mb_out[k])
                        elif isinstance(mb_out[k], np.ndarray):
                            aggregated_non_tensors[k].extend(mb_out[k].tolist())

            revert_indices = None
            if use_dynamic_bsz:
                indices = output_wrapper["indices"]
                indices = list(itertools.chain.from_iterable(indices))
                revert_indices = torch.tensor(get_reverse_idx(indices), dtype=torch.long)
            ...
            # 处理 Tensor
            for k, v_list in aggregated_tensors.items():
                if len(v_list) > 0:
                    cat_tensor = torch.cat(v_list, dim=0) # (Total_Batch, ...)
                    if revert_indices is not None:
                        cat_tensor = cat_tensor[revert_indices]
                    final_dict[k] = cat_tensor.cpu()
                else:
                    final_dict[k] = None
            for k, v_list in aggregated_non_tensors.items():
                if len(v_list) > 0:
                    combined_arr = np.array(v_list, dtype=object)
                    if revert_indices is not None:
                        # numpy indexing
                        revert_idx_np = revert_indices.numpy()
                        combined_arr = combined_arr[revert_idx_np]
                    final_dict[k] = combined_arr
                else:
                    final_dict[k] = None

            final_results_container[0] = final_dict
        ...
        results = final_results_container[0]

        if results["input_ids"] is not None:
            data.batch["input_ids"] = results["input_ids"]
            data.batch["attention_mask"] = results["attention_mask"]
            data.batch["position_ids"] = results["position_ids"]
            
            if results["actions"] is not None:
                data.batch["actions"] = results["actions"]
            if results["scales"] is not None:
                data.batch["scales"] = results["scales"]
            if results["predictor_old_log_probs"] is not None:
                data.batch["predictor_old_log_probs"] = results["predictor_old_log_probs"]

        if results["multi_modal_inputs"] is not None:
            data.non_tensor_batch["multi_modal_inputs"] = results["multi_modal_inputs"]
        if results["multi_modal_data"] is not None:
            data.non_tensor_batch["multi_modal_data"] = results["multi_modal_data"]

        data.batch["ori_input_ids"] = original_data_backup["ori_input_ids"]
        data.batch["ori_attention_mask"] = original_data_backup["ori_attention_mask"]
        data.batch["ori_position_ids"] = original_data_backup["ori_position_ids"]
        data.non_tensor_batch["ori_multi_modal_inputs"] = original_data_backup["ori_multi_modal_inputs"]
        data.non_tensor_batch["ori_multi_modal_data"] = original_data_backup["ori_multi_modal_data"]

        if "raw_prompt_ids" in data.non_tensor_batch:
            data.non_tensor_batch.pop("raw_prompt_ids")
