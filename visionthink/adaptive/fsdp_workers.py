...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if "scale" not in scale_multi_modal_data and hasattr(actor_module, "predictor"):
            actor_module.vision_tower.requires_grad_(False)
            actor_module.embed_tokens.requires_grad_(False)
            actor_module.predictor.requires_grad_(False)

        elif "allfrozen" in scale_multi_modal_data and "scale" in scale_multi_modal_data:
            self.use_orig_params = True
            actor_module.requires_grad_(False)
            actor_module.predictor.requires_grad_(True)
            if self.rank == 0:
                print("[base model] Backbone is set to frozen.")
                print("[predictor model] Other Part is set to frozen.")
                print("[predictor model] Predictor is set to trainable.")

        elif "frozen" in scale_multi_modal_data:
            self.use_orig_params = True
            actor_module.vision_tower.requires_grad_(False)
            actor_module.embed_tokens.requires_grad_(False)
            if self.rank == 0:
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            ###
            if "ispred" in self.config.model.get("scale_multi_modal_data", None):
                data.meta_info["ispred"] = True
            ###
    ...
    def scale_multi_modal(self, data: DataProto):
        ...
        eval_mode = data.meta_info.pop("eval_mode", False)
        adapter_ctx = self.actor.actor_module.disable_adapter() if is_lora else nullcontext()

        # perform scale_multi_modal
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output = self.actor.scale_multi_modal(data=data, eval_mode=eval_mode)
            output = output.to("cpu")
