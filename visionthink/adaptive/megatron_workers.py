...
###
# from visionthink.adaptive.megatron_checkpoint_manager import MegatronCheckpointManager
###
...
from visionthink.adaptive.megatron_actor import MegatronPPOActor
...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor:
            ...
            for model_chunk in actor_module:
                if hasattr(model_chunk, "vision_tower"):
                    model_chunk.vision_tower.requires_grad_(False)
                    if self.rank == 0:
                        print("[actor model] Vision tower is set to not trainable.")
                
                if hasattr(model_chunk, "predictor"):
                    ...
                    if scale_multi_modal_data:
                        model_chunk.predictor.requires_grad_(True)
                        if self.rank == 0:
                            print("[predictor model] Predictor is set to trainable.")
                    else:
                        model_chunk.predictor.requires_grad_(False)
                        if self.rank == 0:
            ...
            if scale_multi_modal_data: 
                ...
                base_lr = optim_config.get("lr", 1e-6)
                pred_lr_scale = self.config.model.get("predictor_lr_scale", 50.0)
                pred_lr = self.config.model.get("predictor_lr", base_lr * pred_lr_scale)
                
                if self.rank == 0:
                    print(f"[Megatron Optim] Base LR: {base_lr}, Predictor LR: {pred_lr}")

                optim_config_pred = copy.deepcopy(optim_config_megatron)
                optim_config_pred.lr = pred_lr
                optim_config_pred.min_lr = optim_config.get("min_lr", 1e-7) * pred_lr_scale 
    ...
    @GPUMemoryLogger(role="scale", logger=logger)
