    ...
    def update_policy(self, data: DataProto):
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                if self.predictor_optimizer is not None:
                    self.predictor_optimizer.zero_grad()
                ###micro_batches[0].non_tensor_batch['ori_multi_modal_inputs'][0]['pixel_values'].shape
                    # micro_batches[0].non_tensor_batch['ori_multi_modal_data'][0]['pixel_values'].shape
                    # micro_batches[0].non_tensor_batch['multi_modal_inputs']
                for micro_batch in micro_batches:
                    micro_batch = micro_batch.to(get_device_id())
                    micro_batch_metrics = {}
                    model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
                    ...
                    #     # self.actor_module.predictor.scorer.layers[1].cross_attn.to_q.weight
                    #     # self.actor_module.vision_tower.blocks[4].attn.qkv.weight
                    #     # self.actor_module.language_model.layers[8].self_attn.v_proj.weight
                    ...
                    if self.predictor_optimizer is not None and predictor_log_prob is not None:
                        ...
                        if "predictor_update_mask" in model_inputs:
                            print("predictor_adv", model_inputs["predictor_advantages"])
                            print("predictor_mask", model_inputs["predictor_update_mask"])
                            sample_advantage = model_inputs["predictor_advantages"]
                            predictor_mask = model_inputs["predictor_update_mask"]

                        else:
                            sample_advantage = (advantages * response_mask).sum(dim=-1) / (response_mask.sum(dim=-1) + 1e-8) # (B,)
                            predictor_mask = torch.ones_like(predictor_log_prob, dtype=torch.bool) # (B,)  (B, T)
