...
from visionthink.adaptive.fsdp_checkpoint_manager import FSDPCheckpointManager
###
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        ###
        scale_multi_modal_data = self.config.model.get("scale_multi_modal_data", None)
        if hasattr(actor_module, "vision_tower"):
            # predictor = actor_module.predictor
            actor_module.vision_tower.requires_grad_(False)
            self.use_orig_params = True
            if scale_multi_modal_data and "frozen" in scale_multi_modal_data:
                actor_module.requires_grad_(False)
                if self.rank == 0:
                    print("[main model] Base Model is set to trainable.")
            if scale_multi_modal_data:
            # if predictor is not None:
                actor_module.predictor.requires_grad_(True)
                if self.rank == 0:
                    print("[predictor model] Predictor is set to trainable.")
            else:
                actor_module.predictor.requires_grad_(False)
                if self.rank == 0:
                    print("[predictor model] Predictor is set to frozen.")
        ###
        ...
        if role == "actor" and optim_config is not None:
            ...
            for name, param in actor_module_fsdp.named_parameters():
                ...
                elif "vision_tower" not in name:
                    actor_params.append(param)
            
            actor_optimizer = build_optimizer(actor_params, optim_config)
            ...
            if scale_multi_modal_data:
                ...
                if len(predictor_params) > 0:
                    predictor_optim_config = optim_config.copy()
                    predictor_optim_config["lr"] = predictor_optim_config["lr"] * 50 if self.config.model.get("predictor_lr", None) is None else self.config.model.get("predictor_lr")
                    predictor_optimizer = build_optimizer(predictor_params, predictor_optim_config)
            ###
            ...
            ###
            def create_scheduler(optimizer, type_str):
                if type_str == "constant":
                    ...
                    raise NotImplementedError(f"LR scheduler type {type_str} is not supported")
            
            actor_lr_scheduler = None
            if actor_optimizer is not None:
                actor_lr_scheduler = create_scheduler(actor_optimizer, lr_scheduler_type)

            predictor_lr_scheduler = None
            if predictor_optimizer is not None:
                predictor_lr_scheduler = create_scheduler(predictor_optimizer, lr_scheduler_type)
            ###
        ...
        return actor_module_fsdp, actor_optimizer, actor_lr_scheduler, actor_model_config, predictor_optimizer, predictor_lr_scheduler
    ...
    def init_model(self):
        ...
        ###
        from visionthink.adaptive.dp_actor import DataParallelPPOActor
        ###
        ...
        if self._is_actor or self._is_rollout:
            ...
            if self._is_offload_optimizer:
                ...
                if self.predictor_optimizer is not None:
                    offload_fsdp_optimizer(optimizer=self.predictor_optimizer)
                ###
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(
                config=actor_cfg, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer, predictor_optimizer=self.predictor_optimizer,
            )
            ###
        ...
        if self._is_actor:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(
                ...
                predictor_lr_scheduler=self.predictor_lr_scheduler,
                ###
            )
    ...
    @DistProfiler.annotate(color="red", role="print_dtype")
    def print_dtype(self, data: DataProto):
        ...
        if self._is_offload_optimizer:
            ...
            if self.actor_optimizer is not None:
                load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=get_device_id())
            if self.predictor_optimizer is not None:
                load_fsdp_optimizer(optimizer=self.predictor_optimizer, device_id=get_device_id())
            ###

        print("=" * 80)
        print("self.actor.actor_module.vision_tower.predictor.dtype", self.actor.actor_module.vision_tower.predictor.mlp[0].weight.dtype)
        print("self.actor.actor_module.vision_tower.dtype", self.actor.actor_module.vision_tower.dtype)
        print("=" * 80)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.predictor_optimizer is not None:
                predictor_lr = self.predictor_lr_scheduler.get_last_lr()[0]
                metrics["predictor/lr"] = predictor_lr.item() if torch.is_tensor(predictor_lr) else predictor_lr
    ...
    def scale_multi_modal(self, data: DataProto):
        ...
        # perform scale_multi_modal
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output = self.actor.scale_multi_modal(data=data)
            output = output.to("cpu")
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during scale_multi_modal", logger=logger)
        ...
        if local_path is None:
            ...
            if self._is_offload_optimizer:
                ...
                if self.actor_optimizer is not None:
                    offload_fsdp_optimizer(self.actor_optimizer)
                if self.predictor_optimizer is not None:
                    offload_fsdp_optimizer(self.predictor_optimizer)
                ###
            return
