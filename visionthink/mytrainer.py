...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        ###
        if "sid" in data.non_tensor_batch.keys():
            ...
            predictor_advantages, predictor_update_mask = compute_predictor_advantage(
                ...
                scale_indices=data.batch["scales"],
                scale_mask=data.batch["scale_mask"],
                filter_invalid_sid=config.get("use_filter_sid", False),
                use_cost=config.get("use_cost", None),
...
class RayPPOTrainer:
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        ###
        if self.config.actor_rollout_ref.model.scale_multi_modal_data:
            reward_model_keys.update(["ori_multi_modal_inputs", "ori_multi_modal_data", "sid"])
        ###
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                ###
                if self.config.actor_rollout_ref.model.scale_multi_modal_data:
                    scale_n = self.config.actor_rollout_ref.model.get("scale_n", 1)
                    if scale_n > 1:
                    ...
                    with marked_timer("scale", timing_raw, color="cyan"):
                        batch = self.actor_rollout_wg.scale_multi_modal(batch)
                ###
